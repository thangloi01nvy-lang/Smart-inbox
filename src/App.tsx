  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-background-dark text-text-main font-mono selection:bg-primary selection:text-white">
      <div className="w-full max-w-3xl min-h-screen flex flex-col relative border-x-2 border-border-harsh bg-background-dark">
        {currentScreen === 'INBOX' && <Inbox onNavigate={setCurrentScreen} onAnalysisComplete={handleAnalysisComplete} />}
        {currentScreen === 'ROSTER' && <ClassRoster onNavigate={setCurrentScreen} />}
        {currentScreen === 'EDIT_CLASS' && <EditClass onNavigate={setCurrentScreen} />}
        {currentScreen === 'EDIT_STUDENT' && <EditStudent onNavigate={setCurrentScreen} />}
        {currentScreen === 'REPORT_GEN' && <ReportGen onNavigate={setCurrentScreen} />}
        {currentScreen === 'ANALYSIS_DETAIL' && <AnalysisDetail onNavigate={setCurrentScreen} analysisResult={analysisResult} />}
        {currentScreen === 'REPORTS' && <Reports onNavigate={setCurrentScreen} reports={reports} onSelectReport={handleSelectReport} />}
      </div>
    </div>
  );
}
